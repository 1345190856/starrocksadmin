...
```bash
# 拉取指定版本
docker pull ghcr.io/annoymous/starrocks-admin:1.0.0
...
docker run -d \
  ...
  -v $(pwd)/data:/app/data \
  -v $(pwd)/logs:/app/logs \
  ghcr.io/annoymous/starrocks-admin:1.0.0
```
...
services:
  starrocks-admin:
    image: ghcr.io/annoymous/starrocks-admin:latest
...
>
> Helm Chart 包只包含 Kubernetes 部署模板，应用镜像从 `ghcr.io/annoymous/starrocks-admin` 拉取。

```bash
# 下载 Helm Chart 包
wget https://github.com/annoymous/starrocksadmin/releases/download/v1.0.0/starrocks-admin-1.0.0.tgz
...
image:
  repository: ghcr.io/annoymous/starrocks-admin
  tag: "1.0.0"
...
|------|------|--------|
| `replicaCount` | 副本数量 | `1` |
| `image.repository` | 镜像仓库 | `ghcr.io/annoymous/starrocks-admin` |
...
| Docker 镜像 | `ghcr.io/annoymous/starrocks-admin:x.x.x` | 多架构镜像（amd64/arm64），由 docker-publish workflow 构建 |
...
Helm Chart 包只包含部署模板（templates/、values.yaml、Chart.yaml 等），**不包含应用程序二进制文件**。部署时会从 `ghcr.io/annoymous/starrocks-admin` 拉取 Docker 镜像。

---

## 相关链接

- [项目主页](https://github.com/annoymous/starrocksadmin)
- [问题反馈](https://github.com/annoymous/starrocksadmin/issues)
- [更新日志](../CHANGELOG.md)

---

**如有问题，欢迎通过annoymous (support@annoymous.com)**
