...
[database]
url = "mysql://root:password@127.0.0.1:3306/starrocks_admin"
...
start_service() {
    ...
    # 设置环境变量
    # export DATABASE_URL="${DATABASE_URL:-mysql://root:password@127.0.0.1:3306/starrocks_admin}"
    ...
    cd "$DIST_ROOT"
    nohup "$BINARY_PATH" --config "$CONFIG_DIR/config.toml" > "$LOG_DIR/starrocks-admin.log" 2>&1 &
    BACKEND_PID=$!
