...
impl SystemFunctionService {
    ...
    pub async fn get_functions(&self, cluster_id: i64) -> ApiResult<Vec<SystemFunction>> {
        ...
        let mut merged_functions: Vec<SystemFunction> = all_functions
            ...
            .map(|mut func| {
                ...
                if func.cluster_id.is_none() {
                    func.cluster_id = Some(cluster_id);
                }
                func
            })
            .collect();
    ...
    pub async fn create_function(
        ...
        let function_id = result.last_insert_id();
        ...
        for order in req.functions {
            ...
            sqlx::query(
                "INSERT INTO system_function_preferences (cluster_id, function_id, category_order, display_order, is_favorited, updated_at)
                 VALUES (?, ?, ?, ?, COALESCE((SELECT is_favorited FROM system_function_preferences f2 WHERE f2.cluster_id = ? AND f2.function_id = ?), false), CURRENT_TIMESTAMP)
    ...
    pub async fn toggle_favorite(
        ...
        sqlx::query(
            "INSERT INTO system_function_preferences (cluster_id, function_id, category_order, display_order, is_favorited, updated_at)
             VALUES (?, ?, 
                     COALESCE((SELECT f2.category_order FROM system_function_preferences f2 WHERE f2.cluster_id = ? AND f2.function_id = ?), ?),
                     COALESCE((SELECT f3.display_order FROM system_function_preferences f3 WHERE f3.cluster_id = ? AND f3.function_id = ?), ?),
                     ?, CURRENT_TIMESTAMP)
             ON DUPLICATE KEY UPDATE
