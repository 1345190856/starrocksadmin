...
impl MySQLPoolManager {
    ...
    pub async fn get_pool(&self, cluster: &Cluster) -> ApiResult<Pool> {
        ...
        // Skip pooling for temporary clusters (ID 0) used in connection tests
        // This avoids cache pollution where different test credentials share the same ID 0 key
        if cluster_id == 0 {
            return self.create_pool(cluster).await;
        }
