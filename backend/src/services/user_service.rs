...
use sqlx::{FromRow, MySqlPool, Transaction, mysql::MySql};
...
impl UserService {
    pub fn new(pool: MySqlPool, casbin_service: Arc<CasbinService>) -> Self {
    ...
    fn compose_user_with_org(
        ...
        let user_response = UserResponse::from_user_with_org(
            user,
            organization_name,
            false,
            is_org_admin,
