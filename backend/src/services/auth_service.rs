...
impl AuthService {
    pub fn new(pool: MySqlPool, jwt_util: Arc<JwtUtil>) -> Self {
        Self { pool, jwt_util }
    }
    ...
    pub async fn to_user_response(&self, user: User) -> ApiResult<UserResponse> {
        ...
        let role_codes = self.get_user_role_codes(user.id).await?;
        Ok(UserResponse::from_user(user, is_super_admin, is_org_admin, role_codes))
    }
    ...
    pub async fn get_user_role_codes(&self, user_id: i64) -> ApiResult<Vec<String>> {
