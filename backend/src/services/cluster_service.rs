...
impl ClusterService {
    pub fn new(pool: MySqlPool, mysql_pool_manager: Arc<MySQLPoolManager>) -> Self {
    ...
    pub async fn create_cluster(
        ...
        let result = sqlx::query(
            "INSERT INTO clusters (name, description, fe_host, fe_http_port, fe_query_port, 
             ...
             is_active, created_by, organization_id, deployment_mode, fe_mapping)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        )
        .bind(&req.name)
        ...
        let cluster_id = result.last_insert_id() as i64;
    ...
    pub async fn update_cluster(
        ...
        if let Some(mapping) = &req.fe_mapping {
            updates.push("fe_mapping = ?");
            params.push(serde_json::to_string(mapping).unwrap_or_default());
        }
