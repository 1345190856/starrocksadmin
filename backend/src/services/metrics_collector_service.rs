    ...
    async fn run_daily_aggregation_for_cluster(&self, cluster_id: i64) -> ApiResult<()> {
        ...
        let snapshots = sqlx::query_as::<_, MetricsAggregation>(
            ...
            SELECT
                ...
                CAST(SUM(query_total) AS SIGNED) as total_queries,
                CAST(SUM(query_error) AS SIGNED) as total_errors,
        ...
        sqlx::query(
            ...
            ON DUPLICATE KEY UPDATE
                avg_qps = VALUES(avg_qps),
                max_qps = VALUES(max_qps),
                min_qps = VALUES(min_qps),
                avg_latency_p99 = VALUES(avg_latency_p99),
                max_latency_p99 = VALUES(max_latency_p99),
                ...
                avg_disk_usage_pct = VALUES(avg_disk_usage_pct),
                max_disk_usage_pct = VALUES(max_disk_usage_pct),
                data_size_end = VALUES(data_size_end),
                data_growth_bytes = VALUES(data_growth_bytes)
            "#,
        )
        .bind(cluster_id)
        .bind(yesterday)
