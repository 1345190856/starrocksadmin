...
pub struct AuditLogConfig {
    ...
    pub table: String,
    /// Enable detailed debug logging for alerts
    pub debug: bool,
}
...
pub struct CommandLineArgs {
    ...
    /// Enable detailed debug logging for alerts (overrides config file)
    #[arg(long, value_name = "BOOL")]
    pub audit_debug: Option<bool>,
}
...
impl Config {
    ...
    fn apply_env_overrides(&mut self) {
        ...
        if let Ok(debug) = std::env::var("APP_AUDIT_DEBUG")
            && let Ok(val) = debug.parse()
        {
            self.audit.debug = val;
            tracing::info!("Override audit.debug from env: {}", self.audit.debug);
        }
    }
    ...
    fn apply_cli_overrides(&mut self, args: &CommandLineArgs) {
        ...
        if let Some(debug) = args.audit_debug {
            self.audit.debug = debug;
            tracing::info!("Override audit.debug from CLI: {}", self.audit.debug);
        }
    }
...
impl Default for DatabaseConfig {
    fn default() -> Self {
        Self { url: "mysql://root:password@127.0.0.1:3306/starrocks_admin".to_string() }
    }
}
