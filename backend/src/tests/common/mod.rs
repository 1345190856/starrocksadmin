...
pub async fn create_test_db() -> MySqlPool {
    let pool = MySqlPoolOptions::new()
        .max_connections(1)
        .acquire_timeout(Duration::from_secs(3))
        .connect("mysql://root:password@127.0.0.1:3306/starrocks_admin_test")
        .await
...
pub async fn setup_test_data(pool: &MySqlPool) -> TestData {
    ...
    // Create test permissions (using INSERT IGNORE to handle duplicates)
    sqlx::query(
        r#"
    ...
    // Create test roles (using INSERT IGNORE to handle duplicates)
    sqlx::query(
        r#"
...
pub async fn setup_multi_tenant_test_data(pool: &MySqlPool) -> MultiTenantTestData {
    // Clear all data
...
pub async fn assign_user_to_organization(pool: &MySqlPool, user_id: i64, organization_id: i64) {
    sqlx::query(
        r#"
...
/// Create a test user
pub async fn create_test_user(pool: &MySqlPool, username: &str) -> i64 {
    sqlx::query(
...
/// Assign role to user
pub async fn assign_role_to_user(pool: &MySqlPool, user_id: i64, role_id: i64) {
...
pub async fn grant_permissions(pool: &MySqlPool, role_id: i64, permission_ids: &[i64]) {
